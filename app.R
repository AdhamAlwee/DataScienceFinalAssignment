...
# Load datasets
ratings <- read.csv('datasets/ratings.csv')
movies <- read.csv('datasets/movies.csv')
links <- read.csv('datasets/links.csv')
...
# UI
  ui <- fluidPage(
    ...
    sidebarLayout(
      ...
      mainPanel(
        div(style = "margin-bottom: 20px; border: 2px solid #4CAF50; padding: 10px; background-color: #f0f8ff; font-weight: bold; text-align: center;",
    ...
    output$recommendation_plot <- renderPlot({
      ...
      if (!is.null(recs) && nrow(recs) > 0) {
        ggplot(recs, aes(x = reorder(title, avg_rating), y = avg_rating)) +
          ...
          coord_flip() +
          labs(title = "Top 10 Recommended Movies", x = "Movies", y = "Average Rating") +
          theme_minimal()
      }
    })
  }
